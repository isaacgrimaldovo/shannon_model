...
def score_tone(texts: pd.Series, model_name: str = DEFAULT_MODEL_NAME) -> pd.DataFrame:
...
def load_or_compute_tone(
    df: pd.DataFrame, cache_path: str | Path, model_name: str = DEFAULT_MODEL_NAME
) -> pd.DataFrame:
    ...
    if not pending.empty:
        scored = score_tone(pending.set_index("nota_id")["cuerpo_texto"], model_name=model_name).reset_index()
